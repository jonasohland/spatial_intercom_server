...
git -C $temp_rep_dir/si_deploy commit -am "deploy-auto-commit @"$(cat package.json | jq -r .version)" from jonasohland/spatial_intercom_server@"$(git rev-parse --short HEAD)
